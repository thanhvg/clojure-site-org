...
- [[https://github.com/clojure/tools.deps.alpha][tools.deps.alpha]] - a
  library providing an API for resolving dependency graphs and building
...
#+BEGIN_QUOTE
  *Note*
  ...
  See the later section on [[#quoting]["Quoting keys and values"]] for
  more details on how to properly quote edn values on the command line.
#+END_QUOTE
...
#+BEGIN_QUOTE
  *Note*
  ...
  aliases for resolving the function (see [[#namespaces]["Namespace
...
See the [[file:repl_and_main.org][clojure.main]] documentation for more
...
maps with the following top-level keys:
...
replacements), config. This merged deps will be included in the runtime
basis and used after this point.
...
See [[file:dep_expansion.org][dep expansion]] for more details on the
dep expansion algorithm.
...
- Maven coordinate: ={:mvn/version "1.2.3"}=
  ...
  - Note: =:classifier= is not supported - add to lib name as specified
    above
...
- Git coordinate:
  ...
  - Required key =:git/url= can be one of the following:
    ...
    - ssh or user@host form urls (including GitHub) - ssh-based access
...
deps.edn file, arg maps for the resolve-deps and make-classpath steps,
and produce the "runtime basis", or "basis" for short. The basis is a
superset of the deps.edn file also containing those args, the lib map,
and the classpath map.
...
operations: =resolve-deps= and =make-classpath=.
...
- Else (no project deps.edn)
  ...
  - Else use =config_dir/.cpcache= (see deps.edn sources for config_dir
    logic, usually =~/.clojure=)
...
- =:replace-deps= - a deps map from lib to coordinate of deps to REPLACE
  the project =:deps=
...
- =:default-deps= - a deps map from lib to coordinate of versions to use
  if none is found
...
The make-classpath modifiers are provided in a map that was described in
a prior section:
...
- If multiple maps with these keys are activated, only the last one will
  be used
...
- =-X:deps tree= - print dependency tree (also see
  [[file:dep_expansion.xml#_tree_printing][tree printing]])

- =-X:deps mvn-pom= - generate (or update an existing) pom.xml with deps
  and paths
...
The =mvn= procurer will look for =:mvn/repos=. The installation deps.edn
configures the default Maven repos:
...
will be checked first for Maven libraries.
...
- tool
  ...
  - =:replace-paths= - vector of path strings that should *replace* the
    project paths
...
- make-classpath
  ...
  - =:extra-paths= - vector of string paths (or keyword aliases to same)
    to add to base paths

  - =:classpath-overrides= - map of lib to string path to replace lib on
    classpath
...
- If function is unqualified, use the namespace in the =:ns-default= key
  in the arg map (if none, this is an error)

- If function is qualified, and the qualifier is an alias in the arg map
  under =:ns-aliases=, use that namespace
...
- unversioned libraries - a file location identifying a jar or directory
  root
