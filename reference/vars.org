...
[[file:refs.xml][Refs]], [[file:agents.xml][Agents]] and
[[file:atoms.xml][Atoms]]. Vars provide a mechanism to refer to a
...
[[file:agents.xml][agents]]) provide "binding conveyance", which allows
...
/(see [[file:namespaces.xml][Namespaces]])/. If a =def= expression does
...
[[file:evaluation.xml][Evaluation]], the compiler attempts to resolve
all free symbols as Vars.

The [[file:special_forms.xml#var][var]] special form or the =#'= reader
macro /(see [[file:reader.xml][Reader]])/ can be used to get an interned
