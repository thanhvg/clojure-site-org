...
- Goodbye lazy-cons, hello lazy-seq
  ...
  - new laziness macro - /*lazy-seq*/
    ...
    - returns a logical collection that implements seq by calling the
      ...
      - invokes the body only the first time seq is called on it, caches
        result

      - will call seq on the body's return value if not already a seq or
        nil

  - The net effect is the creation of a virtual collection that does no
    work until seq is called upon it - fully delayed
...
- Audit your code for nil-punning. The lazy branch has supports
  ...
  value of a lazy sequence in a conditional, and will throw an exception
  if you do. Just build clojure like so:
  ...
  - After you are done, rebuild without the flag, as it will slow things
    down.
