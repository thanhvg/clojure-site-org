...
- When calling methods through
  [[file:xref/../../reference/java_interop.org][Java interop]], the Java
  object is passed in as the first argument. In such cases, =->= is
  useful, for example, to check a string for a prefix:
