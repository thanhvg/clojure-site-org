...
* Pretty-printing using =clojure.pprint=
  :PROPERTIES:
  :CUSTOM_ID: _pretty_printing_using_literal_clojure_pprint_literal
  :END:
...
=clojure.pprint= has a function for that:
...
Finally, for a result which is a sequence of maps (like the above), you
can use
...
* Accessing recent results: =*1=, =*2=, =*3=
  :PROPERTIES:
  :CUSTOM_ID: _accessing_recent_results_literal_1_literal_literal_2_literal_literal_3_literal
  :END:
...
Some expressions won't return a result when you evaluate them, but throw
an [[https://en.wikipedia.org/wiki/Exception_handling][Exception]]
instead. Throwing an Exception is your program saying to you: "something
went wrong when evaluating the expression, and I don't know how to deal
...
stacktrace can be printed using
[[https://clojure.github.io/clojure/clojure.repl-api.html#clojure.repl/pst][clojure.repl/pst]]:
...
-  *Exceptions convey data:* in Clojure programs, it's common to attach
   ...
   message): this is done by creating the Exception via
...
[[https://clojure.github.io/clojure/clojure.java.javadoc-api.html][clojure.java.javadoc]]
lets you view the [[https://en.wikipedia.org/wiki/Javadoc][Javadoc]] of
a class or object. Here is how to view the Javadoc for a Java
[[https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html][regex
Pattern]]:
...
[[https://clojure.github.io/clojure/clojure.inspector-api.html][clojure.inspector]]
lets you open GUI-based visualizations of data, for instance:
...
lets you open any URL in a Web browser, which can be handy for specific
needs.
...
that value.[fn:1] This often happens with values which are obtained via
Java interop.

As an example, we'll create an InputStream object using the
[[https://clojuredocs.org/clojure.java.io][=clojure.java.io=]] lib. If
you don't know what an InputStream is, all the better - the point of
this section is to teach you how to find your footing in uncharted
territory:
...
** Viewing the type hierarchy using =type= and =ancestors=
   :PROPERTIES:
   :CUSTOM_ID: _viewing_the_type_hierarchy_using_literal_type_literal_and_literal_ancestors_literal
   :END:
...
As we saw in the previous section, we can use the
[[https://clojure.github.io/clojure/clojure.java.javadoc-api.html][clojure.java.javadoc]]
...
** Inspecting Java types with =clojure.reflect=
   :PROPERTIES:
   :CUSTOM_ID: _inspecting_java_types_with_literal_clojure_reflect_literal
   :END:
...
how to deal with hairy data structures in the
[[_pretty_printing_using_code_clojure_pprint_code][first section]] of
this chapter: pretty-printing to the rescue! Let's use pretty-printing
to display the methods exposed by =java.io.InputStream= in a table:
...
[fn:1] For instance,
       [[https://docs.datomic.com/on-prem/entities.html][Datomic]] and
       [[https://github.com/tonsky/datascript][DataScript]] Entity
       objects are printed like Clojure maps, even though they are
       significantly different from ordinary maps.
