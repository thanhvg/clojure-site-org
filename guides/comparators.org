...
-  Write either a 3-way comparator or a boolean comparator:
   ...
   -  A 3-way comparator takes 2 values, /x/ and /y/, and returns a Java
      ...
      /x/ comes after /y/, or 0 if they are equal. Use values -1, 0, and
      1 if you have no reason to prefer other return values.
...
* Clojure's default comparator
  :PROPERTIES:
  :CUSTOM_ID: _clojure_s_default_comparator
  :END:
...
-  strings are sorted in
   ...
   of UTF-16 code units. This is alphabetical order (case-sensitive) for
...
-  symbols are sorted first by their namespace, if they have one, and if
   ...
   lexicographically. All symbols that do not have a namespace are
   sorted before any symbol with a namespace.
