...
for maintaining a collection of values in a desired sorted order, e.g a
...
[[https://clojure.github.io/clojure/clojure.core-api.html#clojure.core/compare][compare]]
works well for sorting numbers in increasing order, or strings,
keywords, or symbols, in
[[https://en.wikipedia.org/wiki/Lexicographical_order][lexicographic]]
...
- Ensure that your comparators are based on a
  [[https://en.wikipedia.org/wiki/Total_order][total order]] over the
...
- Write either a 3-way comparator or a boolean comparator:
  ...
  - A 3-way comparator takes 2 values, /x/ and /y/, and returns a Java
    ...
    /x/ comes after /y/, or 0 if they are equal. Use values -1, 0, and 1
...
- Reverse the sort by reversing the order that you give the arguments to
...
- Do not use comparators for sorted sets and maps that treat two values
  as equal, unless you want at most one of those two values to appear in
...
- strings are sorted in
  ...
  order]] (aka dictionary order) by their representation as sequences of
  UTF-16 code units. This is alphabetical order (case-sensitive) for
  strings restricted to the ASCII subset.

- symbols are sorted first by their namespace, if they have one, and if
  they have the same namespace, then by their name. Both the namespace
  and names are compared as their string representations would be,
  lexicographically. All symbols that do not have a namespace are sorted
  before any symbol with a namespace.
...
[[https://docs.oracle.com/javase/8/docs/api/java/text/Collator.html][Collator]]
...
consistent with a [[https://en.wikipedia.org/wiki/Total_order][total
order]] on the values you want to compare.
...
collections
   :PROPERTIES:
   :CUSTOM_ID: _be_wary_of_not_a_number_values_as_compared_values_in_sorted_collections
   :END:

Clojure's default comparator =compare= treats "Not a Number" (=##NaN=)
values as equal to all other numbers. If you call
[[https://clojure.github.io/clojure/clojure.core-api.html#clojure.core/sort][sort]]
on sequences of numbers that contain occurrences of =##NaN=, it might
throw an exception.
...
number is converted to an /int/ behind the scenes using the Java method
